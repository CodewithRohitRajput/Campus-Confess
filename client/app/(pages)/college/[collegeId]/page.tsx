...
export default function CollegeId() {
    ...
    useEffect(() => {
        const getConfessions = async () => {
            const res = await fetch(`${process.env.backend}/confession/${collegeId}/get`, { method: "GET", credentials: "include" });
            const data = await res.json();
            setConfession(data.allConfessions);
        }

        const getCollegeName = async () => {
            const res = await fetch(`${process.env.backend}/college/getname/${collegeId}`, { method: "GET" })
            const data = await res.json();
            setCollegeName(data);
        }
        getCollegeName();
